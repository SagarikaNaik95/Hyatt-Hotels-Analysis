##R script for IST 687 project 
##Sagarika Naik, Tanamay Atkekar, Jimin Hwang, Mohit Shetty

#Showing USA Map of NPS type using the Google Maps API
library(ggmap)
dfentire <- read.table("c:/out-201412.csv", header=TRUE, 
                       sep=",", row.names="id")
...
cleanCsv <- subset(dfentire, dfentire$NPS_Type != "")
...
#As we can see from the data, the maximum NPS type responders are clustered in three big cities- New York, Los Angeles, Chicago


#Cleaning NA's for NY
dfny1<-dfny[-which(is.na(dfny$LENGTH_OF_STAY_C)),]
dfny1$CHILDREN_NUM_C[is.na(dfny1$CHILDREN_NUM_C)]<-0
View(dfny1)

table(dfall1$`Hotel Name-Long_PL`)

#Cleaning NA's for LA
dfla1<-dfla[-which(is.na(dfla$LENGTH_OF_STAY_C)),]
dfla1$CHILDREN_NUM_C[is.na(dfla1$CHILDREN_NUM_C)]<-0
View(dfla1)

#Cleaning NA's for CHI
dfch1<-dfch[-which(is.na(dfch$LENGTH_OF_STAY_C)),]
dfch1$CHILDREN_NUM_C[is.na(dfch1$CHILDREN_NUM_C)]<-0
View(dfch1)

dfall<-rbind(dfny1,dfla1,dfch1)
dfall1<-dfall
View(dfall1)
str(dfall1)

dfprom<-dfall1[dfall1$NPS_Type=="Promoter",]
dfprom<-na.omit(dfprom)

dfpass<-dfall1[dfall1$NPS_Type=="Passive",]
dfpass<-na.omit(dfpass)

dfdetr<-dfall1[dfall1$NPS_Type=="Detractor",]
dfdetr<-na.omit(dfdetr)
...
barplot(hist_ny_states$Freq, names.arg =hist_ny_states$Var1, las=2,  main = "Number of Visitors to NYC" , col=c("darkblue","red"))
